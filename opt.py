...
def objective(parameters):
	...
	time.sleep(3)
	if os.system(f"sudo smartctl -i \"{virtdevid}\" | grep CSL_Virt_MN_01") != 0:
		# wait and retry
		time.sleep(10)
		if os.system(f"sudo smartctl -i \"{virtdevid}\" | grep CSL_Virt_MN_01") != 0:
...
space = [
	[640,1024], # NAND_CHANNEL_BANDWIDTH (MB/s)
	[-1,3400], # PCIE_BANDWIDTH
	[3e4,10e4], # 4KB_READ_LATENCY (ns)
	[3e4,12e4], # READ_LATENCY
	[-1, 3.3e6], # PROG_LATENCY and ERASE_LATENCY
	[5e3, 3e4], # FW_4KB_READ_LATENCY
	[5e3, 3e4], # FW_READ_LATENCY
	[5e3, 2e4], # FW_WBUF_LATENCY0
	[100, 1200], # FW_WBUF_LATENCY1
	[0, 3500], # FW_CH_XFER_LATENCY
	[6144, 65536], # GLOBAL_WB_SIZE (KB) <- Do not set it too low, should be at least MDTS-sized (MDTS=8, >=1MB)
]
...
if __name__ == "__main__":
	...
	LOAD = "./results/checkpoint_240104T1616.pkl"# put file to load (./output/checkpoints/checkpoint 1691380588.pkl), if not loading a previous result from a file, set to 0
	res = skopt.load(LOAD) if LOAD else None
	res = skopt.optimizer.gp_minimize(
		func=objective_opt, dimensions=skopt_dim(space),
		initial_point_generator="hammersly", n_calls=70, n_initial_points=0 if LOAD else 15,
...
#objective([640,3400,76000,76000,3300000,5000,5000,14000,1000,3000,4096]) # Bravo (PCIe 3.0)